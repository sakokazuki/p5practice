...
void setup(){
  ...
  for(int i=0; i<bs.length; i++){
    bs[i] = new BallSet(posX[i], posY[i], width/2, white, ballCol[i]);
  }
}
...
class BallSet{
  ...
  BallSet(float x, float y, int size, color col0, color col1){
    _x = x;
    _y = y;
    _size = size;
    _col[0] = col0;
  ...
  void createBall(){
    ...
    DoubleBall b = new DoubleBall(_x, _y, size, 100, 90, (int)random(0, 360), randomAngle, _col);
    _doubleBalls.add(b);
...
void gifSetup(){
  // GIFアニメ出力の設定
  gifExport = new GifMaker(this, "export_2.gif");
